...
1. [ ルーム名 ] 欄に識別名を入力し、利用するRAGを [ RAG指定 ] 選択から選択します。なお、RAG不要時は「 none 」を選択します。
...
ルーム不要でTakaneに対してプロンプトを入力し回答を得るだけでよい場合には、ルームの設定や管理などの事前実行が必要なWebAPI呼び出しは煩雑に感じてしまいます。  
そのようなユースケースに対するサンプルコードとして「ルームなし会話」モードを実装しています。

### ルームなしの選択
ルームの選択で「(none)」を選択するとチャットルームなしでの利用となります。  
サンプルコード内で画面に表示されている過去履歴を渡しているので、過去履歴を踏まえたやりとりが可能となっておりますが、過去履歴を渡さずに API コールをすれば、必ず入力したプロンプトだけを踏まえた回答となります。

### APIの選択
ルームとして「 (none) 」を選択すると、右横に使用する API を選択する機能を表示します。
「Cohere v2 chat 互換 API」または「Cohere OpenAI Chat Completions 互換 API」が選択できます。

### ルームなしの設定
しているときに、左下の歯車アイコンをクリックすると、推論パラメタの編集が可能です。
1. [ ルーム名 ] は、「 (none) 」固定です。
2. [ ランダム性 ] は、Temperature と呼ばれるもので、0.0～1.0の範囲で0.1刻みで指定可能です。0.0を指定すると毎回同じ回答になり、1.0に近づくにつれてLLMはより様々な探索を行う可能性が高くなります。
3. [ システムプロンプト ] は、推論時のシステムプロンプトを指定します。
4. [ RAG 指定 ] は、推論時に利用する RAG を指定します。なお、RAG不要時は「 none 」を選択します。
5. [OK] をクリックすると変更した内容でルームを更新します。
...
### RAG 参照
ルームなしの設定で RAG を指定したとき、「Cohere v2 chat 互換 API」を選択している場合は、 documents オプションを使って、RAG の検索結果を参照情報として設定して推論します。  
「Cohere OpenAI Chat Completions 互換 API」を選択している場合は、 documents オプションがないため、入力プロンプトの先頭に追加して推論します。
...
| POST | /api/v1/pass-through/takane/v2/chat | Cohere v2 Chat 互換 API |
| POST | /api/v1/pass-through/takane/compatibility/v1/chat/completions | Cohere OpenAI Chat Completions 互換 API |
| POST | /api/v1/retrievers/search | 入力プロンプトからRAG検索 |
